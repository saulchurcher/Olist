...
class Seller:
    ...
    def get_review_score(self):
        ...
        order_order_items = orders.merge(order_items, on='order_id')

        seller_df = data['sellers']

        seller_order_items = order_order_items.merge(seller_df, on='seller_id')

        seller_group = seller_order_items[['seller_id','dim_is_five_star', 'dim_is_one_star', 'review_score']].rename(columns={'dim_is_five_star' : 'share_of_five_stars'}).rename(columns={'dim_is_one_star' : 'share_of_one_stars'})


        seller_group_new = seller_group.groupby('seller_id').mean().round(1)

        return seller_group_new.dropna()
